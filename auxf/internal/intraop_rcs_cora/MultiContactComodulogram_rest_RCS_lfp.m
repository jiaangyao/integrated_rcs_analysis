...
function MultiContactComodulogram_rest_RCS_lfp(lfp, name, surrogate)
...
for chan =1:num_chan
    %% Remove zeros added at the end of the lfp signal
        signal = lfp.contact(chan).signal_ref;
         Fs = lfp.Fs(chan);
...
save([name '_lfp_Com'],'Comodulogram_surr','Comodulogram','Comodulogram_phase','p_surr','Mean_surr','Std_surr','PhaseFreqVector','PhaseFreq_BandWidth','AmpFreqVector','AmpFreq_BandWidth');
