function ETA_spectrogram(name,ecog,trial_onset)
...
% calculate spectrogram for all ecog/LFP
...
xx = find(trial_onset +POST*Fs<length(ecog.contact(1).signal_ref));
trial_onset  = trial_onset (xx);

event_onset = trial_onset;    
trials_ok =1:length(event_onset);

n_data_ch=length(ecog.contact);
n_epochs = length(event_onset);



for i = 1:n_data_ch
    if ~isempty(ecog.contact(i).signal_ref)
    data=ecog.contact(i).signal_ref;
...
% plot TRIAL ONSET spectrogram for all ecog/lfp data 
hf1 = figure;
ff = find(faxis<=50);
...
%     imagesc(taxis,faxis,A2plot(:,:,i),clims1);
    ...
    % axis labels/title
    if i==1
        title([name(1:end-5) '  aligned on image onset   trial onset '  ]);
            xlabel('time (sec)');
            ylabel('frequency (Hz)');
    end
     caxis([0.5 2])
...
saveas(gcf,[name(1:end-5) '_ETAmedian_.fig']);
...
save([name(1:end-5) '_ETA_.mat'], 'S_mag_median', 'S_mag_mean','WINDOW','NOVERLAP','NFFT','FRAME_ADVANCE','PRE','POST','ADD','BL');
end
