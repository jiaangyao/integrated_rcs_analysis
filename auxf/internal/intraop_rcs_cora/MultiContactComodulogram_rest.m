...
function MultiContactComodulogram_rest(ecog, lfp, name, surrogate)
...
for chan =1:num_chan
    ...
    if chan> length(ecog.contact)
         signal = lfp.contact(chan-length(ecog.contact)).signal_ref;
         Fs = lfp.Fs(chan-length(ecog.contact));
    else
