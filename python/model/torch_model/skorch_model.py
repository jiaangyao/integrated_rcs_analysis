...
class SkorchModel(BaseModel):
    def __init__(self, model, architecture_kwargs, training_kwargs):
        ...
        architecture_kwargs = {
            f"module__{k}": v for k, v in architecture_kwargs.items()
        }
        # Skorch prefers unininstantiated model, optimizer, and criterion
        architecture_kwargs["module"] = self.inner_model.type()
        training_kwargs["optimizer"] = self.return_optimizer(
            training_kwargs["optimizer"]
        )
        training_kwargs["criterion"] = self.return_criterion(
            training_kwargs["criterion"]
        )
    ...
    def override_model(self, model_args, model_kwargs) -> None:
        ...
        if "architecture" in config.keys():
            ...
            architecture_kwargs = {
                k: v for k, v in config.items() if k in ARCHITECURE_PARAMS
            }
