...
def calc_high_res_spectra(data_label, label_time_domain, fs, interval, update_rate, stim_level, ch2use):
    ...
    for i in range(len(np.unique(label_time_domain))):
        ...
        keys = np.arange(data_label.shape[1])[data_label.columns.str.contains('key', case=False)]
        keys = keys[ch2use]
        ...
        vec_sig_chunk_label = [data_chunk_curr.iloc[:, keys].values for data_chunk_curr in vec_data_chunk_label]
        vec_time_chunk_label = [data_chunk_curr.loc[:, 'time'].values for data_chunk_curr in vec_data_chunk_label]
        idx_stim = np.arange(data_label.shape[1])[data_label.columns.str.contains('stim', case=False)]

        # now calculate the spectra
        vec_stim_chunk_label = [data_chunk_curr.iloc[:, idx_stim].values for data_chunk_curr in vec_data_chunk_label]
        vec_sig_chunk_label_full = []
        vec_stim_chunk_label_full = []
        vec_time_chunk_label_full = []
        for j in range(len(vec_sig_chunk_label)):
            x, y, z = separete_by_stim(vec_sig_chunk_label[j], vec_stim_chunk_label[j], vec_time_chunk_label[j])
        ...
        bool_too_short = [True if x.shape[0] < NFFT * T else False for x in vec_sig_chunk_label_full]
        vec_sig_chunk_label_full = [x for x, y in zip(vec_sig_chunk_label_full, bool_too_short) if not y]
        vec_time_chunk_label_full = [x for x, y in zip(vec_time_chunk_label_full, bool_too_short) if not y]
        vec_stim_chunk_label_full = [x for x, y in zip(vec_stim_chunk_label_full, bool_too_short) if not y]
        ...
        for j in range(len(stim_level)):
            bool_stim_match = [True if np.all(x == stim_level[j]) else False for x in vec_stim_chunk_label_full]
            sigs_curr = [x for x, y in zip(vec_sig_chunk_label_full, bool_stim_match) if y]
