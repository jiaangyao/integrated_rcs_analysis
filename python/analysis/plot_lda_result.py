...
def load_model_id_results(output):
    acc_python_full = np.stack(
        [output["sinPB"][i]["vec_acc"][:5] for i in range(len(output["sinPB"]))], axis=1
    ).T
    auc_python_full = np.stack(
        [output["sinPB"][i]["vec_auc"][:5] for i in range(len(output["sinPB"]))], axis=1
    ).T
    acc_python_top = [
    ...
    acc_python_full_sfs = np.stack(
        [output["sfsPB"][i]["vec_acc"][:5] for i in range(len(output["sfsPB"]))], axis=1
    ).T
    auc_python_full_sfs = np.stack(
        [output["sfsPB"][i]["vec_auc"][:5] for i in range(len(output["sfsPB"]))], axis=1
    ).T
    acc_python_top_sfs = [
...
def plot_lda_results(str_subject="RCS02", str_side="R"):
    # hard code all paths
    ...
    p_figure_output = pathlib.Path(
        "/home/jyao/Downloads/biomarker_id/figures/model_id/{}".format(str_subject)
    )
    ...
    plt.title(f"Comparison of AUC for Top 5 Power Bands: {str_subject}_{str_side}")
    plt.legend(frameon=False)
