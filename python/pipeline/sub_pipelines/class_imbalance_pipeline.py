        ...
        if imb_config.get('channel_options'):
            ...
            if len(y_list) > 1:
                for i in range(0, len(y_list)-1):
                    ...
                    if groups is not None:
                        assert np.array_equal(groups_list[i], groups_list[i+1]), "Group vectors are not equal after class imbalance correction."
                        assert np.array_equal(y[i], y_group[i]), "Imbalance applied to group labels did not match data labels."
            
            y = y_list[0]
            if groups: groups = groups_list[0]
            ...
            if groups is not None:
                ...
                assert np.array_equal(y_tmp, y_group), "Imbalance applied to group labels did not match data labels."
