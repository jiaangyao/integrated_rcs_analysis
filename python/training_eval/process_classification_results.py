...
def process_and_plot_losses(scores_df, run_dir):
    ...
    WandB_line_plots(
        ...
        x_axis="Epoch",
        lines=["Average Loss"],
        title="Average Epoch Losses Across Folds",
    )

    # Create plot of each individual fold, save locally then log to WandB
    loss_df = loss_df.melt(
        id_vars=["Epoch"], value_vars=folds, value_name="Loss", variable_name="Fold"
    )  # Data needs to be in long format for Plotly
...
def process_and_plot_epoch_metrics(scores_df, run_dir):
    ...
    for col in scores_df.columns:
        ...
        metric_df_agg = metric_df.select(
            [
                pl.col("Epoch"),
                pl.concat_list(*folds)
                .list.drop_nulls()
                .list.mean()
                .alias(f"Average {name}"),
            ]
        )
        # Log averages to WandB
        WandB_line_plots(
            metric_df_agg,
            x_axis="Epoch",
            lines=[f"Average {name}"],
            title=f"Average Epoch {name} Across Folds",
        )

        # Create plot of each individual fold, save locally then log to WandB
        metric_df = metric_df.melt(
            id_vars=["Epoch"], value_vars=folds, value_name=name, variable_name="Fold"
        )  # Data needs to be in long format for Plotly
...
def process_and_log_eval_results_torch(scores, run_dir, epoch_metrics, test=False):
    ...
    scores = {
        (k.split("_", 1)[1:] if "test_" in k else k): v for k, v in scores.items()
    }
            ...
            [
                pl.Series(
                    ...
                    np.vstack(
                        [
                            np.pad(
                                arr, (0, max_length - len(arr)), constant_values=np.nan
                            )
                            for arr in v
                        ]
                    ),
...
def process_and_log_eval_results_sklearn(scores, run_dir, test=False):
    ...
    scores = {
        (k.removeprefix("test_") if "test_" in k else k): v for k, v in scores.items()
    }
