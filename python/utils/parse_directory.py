...
def parse_directory_rcs_sess(p_dir_in):
    """
    Parse a directory and return a list of all RCS session names in the directory.

    :param str p_dir_in: Path to the directory to parse.
    :return: List of RCS session names.
    """
...
def convert_str_sess_to_pd(vec_p_file_in):
    """
    Save the list of session paths to a pandas dataframe.

    :param list vec_p_file_in: List of session paths.
    :return: Pandas dataframe with the session paths.
    """
    ...
    for p_file in vec_p_file_in:
        ...
        str_timestamp = datetime.datetime.fromtimestamp(float_timestamp).strftime('%Y-%m-%d %H:%M:%S')

        # now append to the dataframe
        df_out = df_out.append({'Session': str_sess, 'Timestamp': float_timestamp, 'DateTime': str_timestamp},
                               ignore_index=True)
...
def parse_directory_rcs_sess_to_pd(p_dir_in):
    """
