...
def extract_polars_column_as_ndarray(
    ...
    if isinstance(df.get_column(column_name).dtype, pl.List):
        ...
        if list_width.height > 1:
            raise ValueError("Column contains lists of different lengths. Cannot convert to 2D array")
        else:
            width = list_width.item()
            
        return df.select(pl.col(column_name).cast(
            pl.Array(inner=pl.Float64, width=width)
            )
        ).to_numpy().T
    else:
