...
def combine_hist(vec_pb):
    ...
    vec_pb_flat = np.concatenate([np.concatenate(vec_pb[i]) for i in range(len(vec_pb))])
    ...
    for i in range(len(vec_pb)):
        hist, bin = np.histogram(np.concatenate(vec_pb[i]), bins=np.arange(min_pb, max_pb + 1))
    ...
    hist = hist[bool_out]; edge = edge[bool_out]
