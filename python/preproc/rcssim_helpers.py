...
def rcssim_fft_wrapper(td_data, time_stamps, settings, gain) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    """
    hann_win = rcs.create_hann_window(settings['fft_size'], percent=settings['window_percent'])

    td_rcs = rcs.transform_mv_to_rcs(td_data, gain)

    data_fft, t_pb = rcs.td_to_fft(td_rcs, time_stamps,
        settings['samplingRate'],
        settings['fft_size'], settings['fft_interval'],
        hann_win, interp_drops=False, output_in_mv=False, shift_timestamps_up_by_one_ind=True)

    data_fft_out = rcs.fft_to_pb(data_fft, settings['samplingRate'], settings['fft_size'],
