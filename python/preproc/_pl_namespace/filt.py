...
def process_signal(data, downsample_factor, method="decimate"):
    ...
    if method == "decimate":
        ...
        downsampled_data = resample(
            transformed_data, int(len(data) / downsample_factor)
        )
    elif method == "resample_poly":
        downsampled_data = resample_poly(transformed_data, 1, downsample_factor)
    else:
        raise ValueError(
            "Invalid downsampling method. Choose 'decimate', 'resample', or 'resample_poly'."
        )
...
def butterworth_bandpass(x: TIME_SERIES_T, N: int, Wn: list, fs: int) -> TIME_SERIES_T:
    ...
    return x.map_elements(
        lambda series: pl.Series(butterworth_bandpass_np(series.to_numpy(), N, Wn, fs))
    )


def _bandpass_envelope_downsample(
...
def butterworth_lowpass(x: TIME_SERIES_T, N: int, Wn: list, fs: int) -> TIME_SERIES_T:
    return x.map_elements(
        lambda series: butterworth_lowpass_np(series.to_numpy(), N, Wn, fs)
    )
...
def butterworth_highpass(x: TIME_SERIES_T, N: int, Wn: list, fs: int) -> TIME_SERIES_T:
    return x.map_elements(
        lambda series: butterworth_highpass_np(series.to_numpy(), N, Wn, fs)
    )
